...
async function initDB() {
  ...
  Object.keys(categoryArr).forEach(async (key) => {
    let { korname } = categoryArr[key];
    let { engname } = categoryArr[key];
    ...
    Object.keys(subcategoryArr).forEach(async (key) => {
       { korname } = subcategoryArr[key];
       { engname } = subcategoryArr[key];
      const createdSubcategory = await SubCategory.findOrCreate({ where: { engname }, defaults: { korname, engname, categoryId: createdCategory.id } });
    });
  });
