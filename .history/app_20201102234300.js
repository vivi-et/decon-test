...
async function initMainCategoryTable() {
  ...
  Object.keys(main).forEach(async (key) => {
...
async function initSubCategoryTable() {
  ...
  Object.keys(mainCategory).forEach(async (key) => {
    const { korname } = mainCategory[key];
    const { engname } = mainCategory[key];
  });
}
