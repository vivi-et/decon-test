...
async function initDB() {
  ...
  Object.keys(categoryArr).forEach(async (key) => {
    ...
    Object.keys(subcategoryArr).forEach(async (key) => {
      const { korname } = subcategoryArr[key];
      const { engname } = subcategoryArr[key];
      await SubCategory.findOrCreate({ where: { engname }, defaults: { korname, engname, categoryId: createdCategory.id } });
    });
  });
}
