...
async function initDB() {
  ...
  Object.keys(categoryArr).forEach(async (key) => {
    ...
    Object.keys(subcategoryArr).forEach(async (key) => {
      const sub_korname = subcategoryArr[key].korname;
      const sub_engname = subcategoryArr[key].engname;
      await SubCategory.findOrCreate({ where: { engname: sub_engname }, defaults: { korname: sub_korname, engname: sub_engname, categoryId: createdCategory.id } });
    });
  });
}
